...
while true; do
	read -p "ARE YOU SURE YOU WANT TO DELETE ALL REMOTE IMAGES ON ${DOCKER_REPO}? (y/n)" yn
	case $yn in 
...
if [ ${#imagesarray[@]} -eq 0 ]; then
	...
	echo "Repository ${DOCKER_REPO} is already empty"
	...
	for image in "${imagesarray[@]}"
		do
			echo "Deleting $image..."
			curl --location --request DELETE "https://hub.docker.com/v2/repositories/${DOCKER_REPO}/${image}/" \
