...
public class FileUtil {
	...
	public static DownloadDataMessage createDownloadDocMessage(DownloadDocs downloadDocs, Path parentDirectory,
		...
		DownloadDataMessage downloadDataMessage = new DownloadDataMessage();

		if (apiCategoryType == APICategoryType.ESIGNAPI) {

			downloadDataMessage.setAccountId((String) inputParams.get("inputAccountId"));
			downloadDataMessage.setRecordId((String) inputParams.get("inputEnvelopeId"));
		} else {

			PathParam accountIdParam = ReportAppUtil.findPathParam(downloadDocs.getDownloadParams(),
					AppConstants.DOC_DOWNLOAD_ACCOUNTID_LABEL);

			if (null != accountIdParam) {

				downloadDataMessage.setAccountId((String) inputParams.get(accountIdParam.getParamValue()));
			} else {

				log.warn("RecordId param is null in ruleEngine");
				downloadDataMessage.setAccountId(AppConstants.NOT_AVAILABLE_CONST);
			}

			PathParam recordIdParam = ReportAppUtil.findPathParam(downloadDocs.getDownloadParams(),
					AppConstants.DOC_DOWNLOAD_RECORDID_LABEL);
			if (null != recordIdParam) {

				downloadDataMessage.setRecordId((String) inputParams.get(recordIdParam.getParamValue()));
			} else {
		...
		downloadDataMessage.setFileName(fileName);
		downloadDataMessage.setFileSaveFormat(fileSaveFormat);
		downloadDataMessage.setFolderName(folderName);
		downloadDataMessage.setInputParams(inputParams);
		downloadDataMessage.setParentDirectory(parentDirectory.toString());
