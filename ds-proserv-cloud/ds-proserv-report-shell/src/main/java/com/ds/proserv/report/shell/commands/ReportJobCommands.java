package com.ds.proserv.report.shell.commands;
...
import com.ds.proserv.report.shell.helper.DocuSignShellCommandHelper;
import com.ds.proserv.report.shell.service.BatchTriggerService;
...
public class ReportJobCommands {
	...
	@Autowired
	private DocuSignShellCommandHelper docuSignShellCommandHelper;
	...
	public void generateTables() throws IOException {
		...
		File createTableFile = new File(
				dsCacheManager.prepareAndRequestCacheDataByKey(PropertyCacheConstants.REPORT_CREATETABLE_FILEPATH));
		...
		reportJDBCService.createDynamicTables(tableCreationRequest);
		saveTableCacheData(tableCreationRequest);
	}

	private void saveTableCacheData(TableCreationRequest tableCreationRequest) {
		...
		tableDefinitions.forEach(tableDefinition -> {
			...
			cacheLogDefinitionList.addAll(ReportAppUtil.createCacheLogInfoList(tableDefinition));
		});
		...
		if (null != cacheLogDefinitionList && !cacheLogDefinitionList.isEmpty()) {
			...
			coreCacheDataLogService.saveAllCache(cacheLogInformation);
		}
	}
	...
	private void validateBatchParametersAndTriggerService(String batchStartDateTime, String batchEndDateTime,
		...
		if (!"defaultValue".equalsIgnoreCase(batchStartDateTime)) {
			...
			if (batchStartDateTime.contains("T")) {
				...
				DateTimeUtil.isValidDateTimeByPatternNano(batchStartDateTime);
			} else {
			...
			if (!"defaultValue".equalsIgnoreCase(batchEndDateTime)) {
				...
				if (batchEndDateTime.contains("T")) {
					...
					DateTimeUtil.isValidDateTimeByPatternNano(batchEndDateTime);
				} else {
